...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: normalizeCanonicalPath('/lessons'),
  },
};
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="animate-fade-up flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-blue-100 bg-white/85 p-6 shadow-sm">
        ...
        <Link
          href="/"
          className="rounded-full border border-blue-100 px-4 py-2 text-sm font-semibold text-blue-700 transition-colors hover:bg-blue-50"
        >
          &larr; Back to Home
        </Link>
      </div>
      ...
      <div className="flex items-center justify-center gap-3">
        <Link
          href={`/lessons?page=${Math.max(1, pagination.page - 1)}`}
        ...
        <Link
          href={`/lessons?page=${pagination.page + 1}`}
          className={`rounded-full border px-4 py-2 text-sm font-semibold transition-colors ${
